...
`trash` = Total MSW, `leachate` = Total Tank Loads, `walking_floor` = Total Walking-Floor Loads, `loads` = Total Loads (MSW + tank + WF), `subs` = Total Sub Loads. If a `daily_eod_totals` row exists for the selected date, Totals / Today prefer those five bubbles even when some truck rows exist (partial history). Live today with no snapshot still sums logged loads (MSW + tank + walking-floor). The station picked-up / Closed table stays live-derived. You can also enter the numbers on Today / Totals, or paste that day’s Dispatch Board Google Sheets link and tap **Pull Dispatch Board**. The link stays on the device only — the workbook is never stored in the repo. Web/phone load the Loads footer via `/api/dispatch-board` (Pages Function in production, Vite middleware in `npm run dev`). Desktop uses the Tauri command. Sync is a separate pull/push from `loads` — it never deletes or invents truck rows. Local cache: `chitrader.load-tracker.daily-eod-totals.v1`.
...
- **Roster** (base): in-app **Full Roster** across Burnham + Rockford + Pontiac + Arc + Zion (`hired − status − Vacation VAC`). The Work-Dispatch workbook (`1mdNWIsz7LZauHCccQBB7QzjR-Wo9pukGn8HrmODnPpw`) is a **one-time seed** for empty Driver lists only. Today does **not** read Burnham!L13, Sat-* sums, OOT grids, or Sat-tab banners.
- **Call-offs (app)**: Full Roster status + Vacation auto-VAC + **+ Add** manuals on the Available drivers card (`public.manual_call_offs`). The day-notes call-off workbook is **not** live-pulled. Locked past days may still show sheet pills that were snapshotted before this cutover.
...
Driver **Import empty lists** (and first-open seed of an empty store) may still read the Work-Dispatch workbook if it is shared Viewer-with-link. That path never overwrites a yard list that already has names, and it is not used for Today’s available count. **Refresh** on the Available drivers card syncs cloud days/manuals and recomputes from Full Roster — it does not hit Google.
...
11b. On a date with a `daily_eod_totals` row (SQL insert, Today / Totals → Enter sheet totals, or **Pull Dispatch Board**), End of day / Today TRASH · LEACHATE · WF · LOADS · SUBS match the snapshot — even if some truck rows exist. Total Loads is always MSW + tank + walking-floor. The station table stays load-derived. A day without a snapshot still sums logged loads. Syncing totals must not delete or invent truck rows.
11c. On **Today**, paste that day’s Dispatch Board Google Sheets link (new file each day, share Viewer-with-link) and tap **Pull Dispatch Board**. The LOADS bubble must match the Loads tab Total Loads. The app does not keep a copy of the spreadsheet.
