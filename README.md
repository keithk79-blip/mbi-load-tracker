...
| **Driver** | Chicago-area roster system of record (separate tab, `/driver`). Desktop-first table. Top flipper **Full Roster** / **Sat Roster**, then yard chips **Burnham · Rockford · Pontiac · Arc · Zion**. Full list is truck # + name + optional status (oot/fmla/vac/wc); **+ Add** / × remove persist immediately. Sat list is the same yards with a copy/paste block (`truck name` per line) for email. First-open / **Import from sheet** fills empty yard lists from the available-drivers workbook (no sheet writeback). Local `chitrader.load-tracker.driver-roster.v1` plus Supabase `driver_roster_entries` (run **`Load-Tracker-driver-roster.sql`**). Does **not** replace Today’s L13 / Sat-sum available-driver fetch. |
...
10e. Open **Driver** (sidebar or `/driver`). Confirm Full / Sat grouping, five yard chips, a driver count, and a table (not mobile cards). **+ Add** a truck+name, then × remove it. Switch to Sat Roster, **Copy list**, and confirm newline `truck name` lines. **Import from sheet** fills empty yards only. Today’s available-drivers card still refreshes from L13 / Sat-sum.
