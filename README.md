...
| **Driver** | Chicago-area roster system of record (separate tab, `/driver`). Desktop-first table. Top flipper **Full Roster** (everyone **hired** at that yard) / **Sat Roster** (Saturday planning subset), then yard chips **Burnham · Rockford · Pontiac · Arc · Zion**. Full rows are employee # + name + optional **unavailability** (oot/fmla/vac/wc) — marked drivers stay on the hired list but count as out (`hired − full-day status − Vacation VAC`). Vacation-tab names for the selected **as-of** week are marked **Vac** automatically (derived; stored oot/fmla/etc. stay on the row). **+ Add** / × remove persist immediately. Sat list is copy/paste (`emp# name` per line) for email. First-open / **Import empty lists** fills empty yard lists from the available-drivers workbook (no sheet writeback). **Refresh** on Available drivers does not hit Google. Local `chitrader.load-tracker.driver-roster.v1` plus Supabase `driver_roster_entries` (run **`Load-Tracker-driver-roster.sql`**). Today’s available-drivers card uses this Full Roster (all yards) as the base — not L13. |
...
10e. Open **Driver** (sidebar or `/driver`). Confirm Full / Sat grouping, five yard chips, and a hired table (not a separate unavailable list). Full Roster shows hired / out / available for the **as-of** date. A name on the Vacation tab for that week shows a **Vac** chip and counts as out without overwriting a stored OOT/FMLA mark. **+ Add** an employee # + name, then × remove it. Switch to Sat Roster, **Copy list**, and confirm newline `emp# name` lines. **Import empty lists** fills empty yards only. **Refresh** on Available drivers must not hit Google. Today’s available-drivers card matches Full Roster (all yards) minus leftover full-day offs — not L13.
