...
- **Manual same-day offs**: on the Available drivers card, **+ Add** under Call offs stores an ad-hoc name for the selected Chicago day (Call Off / P-Day / Ok’d Off / NCNS). Pills show the name only; color is blue / green / yellow / red. Sheet names stay; manuals are additive and can be removed (no sheet writeback). Same name on the same day is rejected. Manuals persist in the driver-days `localStorage` key and, with Supabase, `public.manual_call_offs` (run `supabase/migrations/20260908024500_manual_call_offs.sql`). All four types subtract from the available-drivers tally (weekday `L13 − offs`, and Saturday yard sum − manual offs). Sheet Reason maps to those four colors when it matches; other full-day sheet reasons use Call Off (blue).
