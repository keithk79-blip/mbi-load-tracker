...
10e. Open **Driver** (sidebar or `/driver`). Confirm Full / Sat grouping, five yard chips, and a hired table (not a separate unavailable list). Full Roster shows hired / out / available for the **as-of** date. A name on the Vacation tab for that week shows a **Vac** chip and counts as out without overwriting a stored OOT/FMLA mark. **+ Add** an employee # + name, then × remove it. Switch to Sat Roster: an empty Sat list for a yard with Full hires should seed from Full (emp# + name). Confirm the dense multi-column grid (not one tall row per driver), **Copy list** still newline `emp# name`, and **Reset to full roster** recopies that yard only. **Import empty lists** fills empty yards only. **Refresh** on Available drivers must not hit Google. Today’s available-drivers card matches Full Roster (all yards) minus leftover full-day offs — not L13.
