...
`available = max(0, Full Roster available − leftover full-day offs that apply that Chicago calendar day)`

- **Roster** (base): in-app **Full Roster** across Burnham + Rockford + Pontiac + Arc + Zion (`hired − status − Vacation VAC`). Burnham!L13 and Sat-* sum cells are **not** read for this tally.
...
- **Manual same-day offs**: on the Available drivers card, **+ Add** under Call offs stores an ad-hoc name for the selected Chicago day (Call Off / P-Day / Ok’d Off / NCNS / Late/Early). Pills show the name only; color is blue / green / yellow / red / orange. Sheet names stay; manuals are additive and can be removed (no sheet writeback). Same name on the same day is rejected. Manuals persist **per Chicago calendar date** in the driver-days `localStorage` key and, with Supabase, `public.manual_call_offs`. Switching the day picker shows that date’s pills and available count — not today’s list. Past days stay visible and editable (persist, not freeze). If that table is missing, desktop chips stay in LocalStorage and phones see nothing — paste and Run **`Load-Tracker-manual-call-offs.sql`** in the Supabase SQL Editor once (same DDL is also in `Load-Tracker-sync-tables.sql`), then open desktop logged in so Late/Early push, then hard-refresh the phone. Call Off, P-Day, Ok’d Off, and NCNS subtract from the available-drivers tally when that name is not already out on Full Roster / Vacation VAC (weekday Full Roster − offs; Saturday uses the same Full Roster base with the Sat-worklist manual-only subtract unless Sat tabs mark a full-mandatory day). Late/Early is orange status only and does **not** reduce available drivers or the day `drv` tally. Sheet Reason maps to those colors when it matches; other full-day sheet reasons use Call Off (blue). Cloud upsert/fetch/delete failures show on the Available drivers card instead of failing silently.
- **Lock each Chicago calendar day** after it is first computed. Today can refresh from Full Roster + call-offs until midnight Chicago; yesterday and earlier stay frozen even if the roster or the call-off tab changes. A day that was never snapshotted is computed once from the current roster, then locked. Snapshots live in `localStorage` (`chitrader.load-tracker.driver-days.v1`) and, with Supabase, `public.driver_availability` (run `supabase/migrations/20260905003000_driver_availability.sql`). `base` on those rows is the Full Roster available count, not L13.

The browser cannot read a **private** sheet. CHItrader should keep both workbooks as **Share → Anyone with the link → Viewer** (already how the live sheets are set). The Vite / Vercel `/sheets/offs` proxy fetches the call-off CSV so the phone web app does not hit Google CORS. The Tauri Windows app talks to Google directly. Last successful call-off pull is cached in `localStorage` (`chitrader.load-tracker.drivers.v1`). **Refresh call-offs** re-pulls the call-off tab (and Sat-tab full-mandatory flag) for **today only** — it does not read L13.
...
| **Driver** | Chicago-area roster system of record (separate tab, `/driver`). Desktop-first table. Top flipper **Full Roster** (everyone **hired** at that yard) / **Sat Roster** (Saturday planning subset), then yard chips **Burnham · Rockford · Pontiac · Arc · Zion**. Full rows are truck # + name + optional **unavailability** (oot/fmla/vac/wc) — marked drivers stay on the hired list but count as out (`hired − full-day status − Vacation VAC`). Vacation-tab names for the selected **as-of** week are marked **Vac** automatically (derived; stored oot/fmla/etc. stay on the row). **+ Add** / × remove persist immediately. Sat list is copy/paste (`truck name` per line) for email. First-open / **Import from sheet** fills empty yard lists from the available-drivers workbook (no sheet writeback). Local `chitrader.load-tracker.driver-roster.v1` plus Supabase `driver_roster_entries` (run **`Load-Tracker-driver-roster.sql`**). Today’s available-drivers card uses this Full Roster (all yards) as the base — not L13. |
...
10b. Open **Analytics**. Confirm year-to-date load count, **available drivers** (today live from Full Roster minus leftover offs), a transfer-station donut, a 21-day bar chart, a day list with loads and driver counts, and YTD transfer / landfill / commodity groups. Tap **Refresh call-offs** — today may change, locked past weekdays must not.
...
10e. Open **Driver** (sidebar or `/driver`). Confirm Full / Sat grouping, five yard chips, and a hired table (not a separate unavailable list). Full Roster shows hired / out / available for the **as-of** date. A name on the Vacation tab for that week shows a **Vac** chip and counts as out without overwriting a stored OOT/FMLA mark. **+ Add** a truck+name, then × remove it. Switch to Sat Roster, **Copy list**, and confirm newline `truck name` lines. **Import from sheet** fills empty yards only. Today’s available-drivers card matches Full Roster (all yards) minus leftover full-day offs — not L13.
